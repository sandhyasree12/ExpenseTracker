...
def add_expense_route():
    ...
    date = datetime.now().strftime("%Y-%m-%d")  # Use current date as default

    # Call function to add the expense with the current date
...
def add_income_route():
    ...
    # Call function to add the income with the current date
...
def analytics():
    ...
    # Get total expenses by category, now including the date of expense
    cursor.execute('''
        SELECT category, SUM(amount), date FROM expenses GROUP BY category
    ''')
